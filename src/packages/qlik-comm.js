...
const { environmentExists } = require("./initialChecks");
...
const setScript = async function ({
  ...
  try {
    ...
    await doc.setScript("// Some script!");
    spinner.stop(true);
...
const checkScriptSyntax = async function ({
  environment,
  variables,
  script,
  debug,
...
async function createQlikSession({ environment, variables, debug }) {
  ...
  let url = `wss://${environment.host}/app/${
    environment.appId
  }/identity/${+new Date()}`;

  if (qsEnt.error) return qsEnt;

  if (authenticationType == "web") {
    url += `?qlikTicket=${qsEnt.message}`;
  }
...
const handleAuthenticationType = {
    ...
    if (variables.isHomeConfig && environment.authentication.encoding) {
      if (!isBase64(variables.QLIK_PASSWORD)) {
        return {
          error: true,
          message:
            'Please do not store passwords in plain text! Use "qlbuilder encode" to get the encoded version of the password and update the yml entry',
        };
      }
  ...
  web: async function (environment, variables) {
    return new Promise(async (resolve, reject) => {
      const engineHost = `wss://${environment.environment.host}/app/engineData?reloadURI=http://localhost:3000`;
      ...
      session.on(
        ...
        async function (data) {
          open(data.loginUri);
          ticket = webServer.start().then((ticket) => {
            return session.close().then(() => {
              resolve({ error: false, message: ticket });
            });
          });
        }
      );
